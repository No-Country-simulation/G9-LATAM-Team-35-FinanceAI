...
`oci_service.py` necesita dos archivos locales que **no se suben al repo** (ya excluidos en `.gitignore` y `.dockerignore`):

1. **`.config`** — credenciales de autenticación con OCI, en formato INI. Usa `.config.example` como plantilla:
...
2. **`clave.pem`** — la llave privada referenciada en `key_file` arriba. Se descarga desde la consola de OCI al generar la API key del usuario.

Ambos archivos van en la raíz de `data-science/` (junto a `README.md`), y **nunca se comitean**. Cada quien necesita los suyos propios generados desde su propia cuenta de OCI.

Por ahora la carga de modelos sigue usando los `.joblib` locales en `models/` (bloque activo en `prediction_wrapper.py`); los bloques que cargan desde OCI (en RAM o con cache) están comentados y listos para activarse cuando el bucket esté configurado.

## Uso con Docker

1. **Construir la imagen** (desde `data-science/`, donde vive el `Dockerfile`):

```bash
   docker build -t financeai-wrapper .
```

2. **Ejecutar el contenedor**, montando `.config` y `clave.pem` como volúmenes (no viajan dentro de la imagen por seguridad, ver `.dockerignore`):

```bash
   docker run -p 8000:8000 \
     -v $(pwd)/.config:/app/.config \
     -v $(pwd)/clave.pem:/app/clave.pem \
     financeai-wrapper
```

En Windows (PowerShell), reemplazar `$(pwd)` por `${PWD}`.

3. Igual que en local, ir a `http://127.0.0.1:8000/docs` o `http://localhost:8000/docs` para probar los endpoints vía Swagger UI.

Si no vas a usar OCI todavía y solo quieres probar el contenedor con los modelos locales, puedes omitir los `-v` de `.config`/`clave.pem` — el wrapper sigue funcionando porque la carga desde OCI está comentada por default.
